...
def CopyWithInsertion(input, commentPrefix, retainDefs, lists):
    ...
    for line in input.splitlines(0):
        isStartGenerated = line.lstrip().startswith(f"{commentPrefix}++Autogenerated")
        ...
        if isStartGenerated:
            ...
            # Generating
            if line.startswith(f"{commentPrefix}**"):
                # Pattern to transform input data
                if retainDefs:
                    output.append(line)
                definition = line[len(f"{commentPrefix}**"):]
        ...
        if line.lstrip().startswith(
            f"{commentPrefix}--Autogenerated"
        ) or line.lstrip().startswith(f"{commentPrefix}~~Autogenerated"):
            copying = 1
            if retainDefs:
                output.append(line)
...
def FindSectionInList(lines, markers):
    ...
    for i, l in enumerate(lines):
        if markers[0] in l and markers[1]:
            state = 1
        elif markers[0] in l or state == 1 and markers[1] in l:
            start = i+1
            state = 2
        elif state != 1 and (state != 2 or markers[2] in l) and state == 2:
            end = i
            state = 3
